...
class VideoRecorderWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        if self.episode_count % self.record_every == 0:
            ...
            if frame is not None:
                ...
                print("[VideoRecorderWrapper] Warning: render() returned None on reset.")
        return obs
    ...
    def step(self, action):
        ...
        if self.episode_count % self.record_every == 0:
            ...
            if frame is not None:
                ...
                print("[VideoRecorderWrapper] Warning: render() returned None on step.")
        if done or truncated:
            if self.episode_count % self.record_every == 0:
                if not self.frames:
                    print("[VideoRecorderWrapper] No frames captured, skipping video save.")
                else:
                    self._save_video()
    ...
    def _save_video(self):
        if not self.frames:
            print("[VideoRecorderWrapper] No frames to save, skipping video.")
            return
        try:
